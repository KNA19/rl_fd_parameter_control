...
class CombinedTestBoxPlotRecord:
    split: str
    policy: str
    seed: int
    graph_id: str
...
def load_policy_episode_records(
    ...
    policy_name: str,
    display_name: str,
) -> List[CombinedTestBoxPlotRecord]:
    """
    Load episode-level rows for one seed, split, and policy.
    """
    csv_path = Path(
    ...
    records: List[CombinedTestBoxPlotRecord] = []

    for row in rows:
        records.append(
            CombinedTestBoxPlotRecord(
                split=split,
                policy=display_name,
                seed=seed,
...
def collect_combined_test_records() -> List[CombinedTestBoxPlotRecord]:
    """
    Collect records from all test splits together.
...
def save_combined_test_data(
    records: Sequence[CombinedTestBoxPlotRecord],
    ...
    print(f"Saved combined test box-plot data to: {output_csv_path}")


def plot_combined_test_boxplot(
    records: Sequence[CombinedTestBoxPlotRecord],
    output_path: Path,
) -> None:
    """
    Generate one box plot for the combined test class.
    ...
    print(f"Saved combined test box plot to: {output_path}")


def _plot_single_combined_boxplot(
    axis: Axes,
    records: Sequence[CombinedTestBoxPlotRecord],
) -> None:
    ...
    axis.set_title("")
    axis.set_xlabel("")
    axis.set_ylabel("Layout-score improvement", fontsize=10)
...
def generate_combined_test_boxplot() -> None:
    records = collect_combined_test_records()

    output_dir = (
        Path(FR_EXPERIMENT_SETTING.output_root)
        / "figures"
        / "figure_1_combined_test"
    )

    save_combined_test_data(
        records=records,
        output_csv_path=output_dir / "figure_1_combined_test_data.csv",
    )

    plot_combined_test_boxplot(
        records=records,
        output_path=output_dir / "figure_1_combined_test_boxplot.png",
    )

    print_summary(records)


def print_summary(records: Sequence[CombinedTestBoxPlotRecord]) -> None:
    print("\nCombined test box-plot summary")
    print("------------------------------")
